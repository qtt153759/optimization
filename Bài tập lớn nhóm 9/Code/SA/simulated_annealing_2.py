...
var = input("Enter the instance name (e.g. nl8): ")
maxP = str(input("Max P(enter to default is 1250): "))
maxR = str(input("Max R(enter to default is 10): "))
maxC = str(input("Max C(enter to default is 1000): "))
T = str(input("Input T (enter to default is max distance): "))
...
maxP = int(maxP) if maxP.strip().isdigit() else 1250 
maxC = int(maxC) if maxC.strip().isdigit() else 1000 
maxR = int(maxR) if maxR.strip().isdigit() else 10 
...
T = float(T) if T.strip().isdigit() else float(np.amax(distance_matrix))
...
def simulated_annealing(maxP,maxC,maxR,T,beta,weight,teta):
    ...
    np.savetxt('test-output-'+str(var)+'-' +str(e) +".txt".strip(), S,fmt="%d", delimiter=' ')
